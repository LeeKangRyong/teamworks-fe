import { TeamStatus } from "@/shared/ui/project/base";
import { SendMessage } from "@/features/project/team";

interface Props {
    team: string;
    num: number;
    recent: string;
    status: string;
    desc: string;
    onClick?: () => void;
    onSendMessage?: () => void;
}

export function TeamManageList({ team, num, recent, status, desc, onClick, onSendMessage }: Props) {
    return (
        <div
            className="flex flex-row items-center py-1 hover:bg-gray-10 pl-2 -ml-2 border-b-1 border-gray-10"
            onClick={onClick}
        >
            <div className="w-40 flex-shrink-0">
                <p className="text-body-s text-secondary-70 text-left">팀 {team}</p>
            </div>
            <div className="w-20 flex-shrink-0">
                <p className="text-body-s text-secondary-70 text-left">{num}명</p>
            </div>
            <div className="w-30 flex-shrink-0">
                <p className="text-body-s text-secondary-70 text-left">{recent}</p>
            </div>
            <div className="w-30 flex-shrink-0">
                <TeamStatus status={status} />
            </div>
            <div className="flex-1 min-w-0 pr-2">
                <p className="text-body-s text-secondary-70 text-left">{desc}</p>
            </div>
            <div onClick={(e) => e.stopPropagation()}>
                <SendMessage onClick={onSendMessage} />
            </div>
        </div>
    );
}
