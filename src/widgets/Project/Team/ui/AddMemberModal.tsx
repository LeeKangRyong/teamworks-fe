...
interface Props {
    onClose?: () => void;
    onConfirm?: (selectedParticipants: unknown[]) => void;
}

export function AddMemberModal({ onClose, onConfirm }: Props) {
    const [selectedParticipants, setSelectedParticipants] = useState<unknown[]>([]);
    const [selectedCount, setSelectedCount] = useState(0);

    // useCallback으로 함수 메모이제이션
    const handleSelectionChange = useCallback((selectedMembers: unknown[], count: number) => {
