...
export function ProjectForm({ type, projectId }: Props) {
    ...
    const { project, isLoading: isLoadingProject } = useProjectDetail(type === "update" ? projectId : undefined);
    ...
    useEffect(() => {
        if (type === "update" && project) {
            ...
            setMaxTeamSize(project.maxTeamSize || 4);
            setMaxParticipants(project.maxParticipants || 100);

            if (project.startDate) {
                const dateOnly = project.startDate.split('T')[0];
                setStartDate(dateOnly.replace(/-/g, '/'));
            }

            if (project.endDate) {
                const dateOnly = project.endDate.split('T')[0];
