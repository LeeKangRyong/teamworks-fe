import type { Chat, Message, ChatUserType } from '../model/types';

// Chat에 name/team 없음 → 표시용 확장 타입
 ...
 * 채팅방 검색 (이름, 팀명으로)
 */
export const searchChats = (users: ChatUser[], searchTerm: string): ChatUser[] => {
 ...
 * 채팅 배지 수 가져오기
 */
export const getChatBadge = (chats: Chat[], userId: string): number => {
 ...
 * 채팅방 타입별 필터링
 */
export const filterChatsByType = (users: ChatUser[], chatType: ChatUserType | 'all'): ChatUser[] => {
 ...
 * 채팅방 목록을 표시용 데이터로 변환
 */
export const mapChatsToDisplay = (users: ChatUser[], chats: Chat[]): ChatUser[] => {
