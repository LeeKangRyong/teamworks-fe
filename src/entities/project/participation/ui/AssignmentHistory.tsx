...
export function AssignmentHistory() {
    const sortedSubmits = [...participantSubmitsData].sort((a, b) => {
        ...
        const fullDateA = new Date(2000 + yearA, monthA - 1, dayA, ...timeA.split(':').map(Number) as [number, number]);
        const fullDateB = new Date(2000 + yearB, monthB - 1, dayB, ...timeB.split(':').map(Number) as [number, number]);
